...
*/
	...
	/*
	void drawPixel(uint32_t* framebuffer, int x, int y, int width) {
	    framebuffer[y * width + x] = 0x000000; // Set pixel to black
	}
	*/
	...
	*/
	...
	/*
	for (b = 0; b < (int)bytes; b++) {
		mem_ptr = fb->mem_start + b;
		*mem_ptr = 0x0;
	}
	*/
	/*
	for (y = 0; y < fb->screeninfo.yres; y++) {
	  for (x = 0; x < fb->screeninfo.xres; x++) {
	      mem_ptr = fb->mem_start;
	      mem_ptr += x * (fb->screeninfo.bits_per_pixel/8);
	      mem_ptr += y * (fb->screeninfo.bits_per_pixel/8) * fb->screeninfo.xres;
		  
		  printf("%lu", (uintptr_t)mem_ptr);
	      *mem_ptr = fb_pixel;
	  }
	}
	*/
